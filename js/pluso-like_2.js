(function(w, d) {
    ...
    var lang = function(elem, service, title) {
        ...
        if (title) {
            ...
            return pluso.lang[language] && pluso.lang[language][service] || pluso.lang["ru"] && pluso.lang["ru"][service] || service
    };
    ...
    if (w.pluso && typeof w.pluso.start == "function")
        return;
    if (!w.pluso)
        w.pluso = {};
    pluso.cnt = 0;
    ...
    pluso.html = function(elem, debug) {
        if (elem.pluso)
            return false;
        if (elem.className.split(" ").indexOf("pluso-skip") > -1)
            return false;
        ...
        if (services)
            ep.services = services.split(",");
        iterate(ep.params, function(key, obj) {
            var value = elem.getAttribute("data-" + key);
            if (value)
                obj[key] = value
        });
        if (ep.params.url.split("://").length < 2)
            ep.params.url = prot + "://" + ep.params.url;
        if (ep.params.url.indexOf("#") == -1 && ep.params.url.split("/").length < 4)
            ep.params.url += "/";
                ...
                if (t.className.substr(0, 12) == "pluso-market") {
                    ...
                    o.onclick = function(e) {
                        if (!e)
                            var e = window.event;
                        var t = e.target || e.srcElement;
                        if (t.className == "pluso-overlay-background")
                            o.parentNode.removeChild(o)
                    }
                    ;
    ...
    pluso.build = function(elem, debug) {
        ...
        if (!pluso.counter[elem.pluso.params.url])
            pluso.counter[elem.pluso.params.url] = 0;
        elem.counter.title = pluso.counter[elem.pluso.params.url];
        elem.counter.innerHTML = numk(pluso.counter[elem.pluso.params.url]);
        if (!pluso.counters[elem.pluso.params.url])
            pluso.counters[elem.pluso.params.url] = [];
    ...
    pluso.start = function(el) {
        ...
        for (var ei = 0, ll = el.length; ei < ll; ei++) {
            if (pluso.build(el[ei])) {
                var params = "";
                if (el[ei].pluso.options.sepcounter)
                    params += "&soc=1";
                if (!pluso.cnt++)
                    params += "&first=1";
    ...
    pluso.share_bookmark = function(ep) {
        if (typeof window.sidebar == "object" && typeof window.sidebar.addPanel == "function") {
            ...
            prompt("РЎРєРѕРїРёСЂСѓР№С‚Рµ Рё РґРѕР±Р°РІСЊС‚Рµ СЌС‚Сѓ СЃСЃС‹Р»РєСѓ РІ Р—Р°РєР»Р°РґРєРё", ep.params.url)
    }
    ;
    ...
    pluso.more = function(xy, elem) {
        if (elem.box) {
            ...
            div.onmouseout = function(ev) {
                ...
                if (e && e.tagName && e.tagName.toUpperCase() == "B")
                    e = e.parentNode;
        ...
        if (elem.box && elem.box.style.display == "block")
            return;
        ...
        if (x < 0)
            x = 10;
        else if (x + 310 > ws.width)
            x = ws.width - 350;
        if (y < 0)
            y = 5;
        else if (y + 265 > ws.height)
            y = ws.height - 265;
        div.style.left = x + "px";
        div.style.top = y + "px";
        if (elem.box) {
    ...
    pluso.setCounter = function(c, url, buy, counters, advType) {
        ...
        if (typeof pluso_counter == "function")
            pluso_counter(c, url);
        var du = false;
        if (elements("pluso")[0])
    ...
    ready(function() {
        pluso.start();
        if (typeof pluso_ready == "function")
            pluso_ready()
    })
}
)(window, document);
